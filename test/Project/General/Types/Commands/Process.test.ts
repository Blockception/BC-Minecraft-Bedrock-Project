...
scoreboard players set global id 0`
  ...
  it("animation_controller", ()=>{
    ...
    const doc : TextDocument = {
      ...
      getText: ()=>`"controller.animation.example": {
        "states": {
          "default": {
            "transitions": [
              { "A": "query.has_armor_slot(0)" },
              { "B": "query.has_armor_slot(1)" },
              { "C": "query.has_armor_slot(2)" },
              { "D": "query.has_armor_slot(3)" }
            ]
          },
          "A": {
    ...
    expect(P.BehaviorPacks.animations.get("controller.animation.example")).to.not.be.undefined;
    expect(P.General.tags.count()).to.equal(1);
    expect(P.General.objectives.count()).to.equal(1);
    expect(P.General.fakeEntities.count()).to.equal(1);
    expect(P.General.tickingAreas.count()).to.equal(2);
